...
VPN_CONNECT="nordvpn connect united_states"
...
COLOR_CONNECTED="#8ab1ff"
COLOR_CONNECTING="#f7ca7a"
COLOR_DISCONNECTED="#f37f97"
...
# continually reports connection status
	if [ "$VPN_STATUS" = "$CONNECTED"  ]; then
		if [ "$@" ] && [ "$1" == "--no-geoip" ]; then
			...
			echo "%{F$COLOR_CONNECTED}􀙧%{F-} $(ip_address_lookup)"
		fi
	elif [ "$VPN_STATUS" = "$CONNECTING" ]; then
		echo "%{F$COLOR_CONNECTING}􀙧%{F-} Connecting..."
	else
		echo "%{F$COLOR_DISCONNECTED}􀙧%{F-} Disconnected"
	fi
}
